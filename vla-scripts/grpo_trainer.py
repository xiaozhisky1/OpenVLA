...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        coef_1 = torch.exp(per_token_logps - old_per_token_logps) # 表示当前策略与旧策略的比值
        coef_2 = torch.clamp(coef_1, 1 - self.epsilon, 1 + self.epsilon) # 限制策略更新幅度，避免剧烈变化
        per_token_loss1 = coef_1 * advantages.unsqueeze(1)
        per_token_loss2 = coef_2 * advantages.unsqueeze(1)
        per_token_loss = -torch.min(per_token_loss1, per_token_loss2) # per_token_loss 取较小值（即保守更新）

        # Add KL penalty if beta > 0 计算 KL 散度惩罚
        if self.beta > 0:
